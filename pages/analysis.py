...
def select_folder(): 
    ...
    base_directory = '/home/site/wwwroot/responses'
    #base_directory = '/Users/orlando/Desktop/Tesi/TEST/jail-test/results'
    
    # Elenca tutte le sottocartelle nella directory di base
    try:
        subfolders = [f for f in os.listdir(base_directory) if os.path.isdir(os.path.join(base_directory, f))]
    except FileNotFoundError:
        st.error(f"La directory di base specificata non esiste: {base_directory}")
        subfolders = []

    # Aggiungi un'opzione per nessuna selezione
    subfolders.insert(0, "Seleziona una cartella")

    # Widget per selezionare una cartella
    selected_folder = st.selectbox("Scegli una cartella:", subfolders)

    # Mostra il path completo della cartella selezionata
    if selected_folder != "Seleziona una cartella":
        final_path = os.path.join(base_directory, selected_folder)
        st.write(f"Percorso selezionato: {final_path}")

        # Verifica se il path esiste
        if os.path.exists(final_path):
            st.success(f"Il percorso selezionato esiste: {final_path}")
            return final_path
            # Aggiungi codice per visualizzare i dati della cartella selezionata
        else:
            st.error(f"Il percorso selezionato non esiste: {final_path}")
    else:
...
folder = f"{base_folder}/folder_results"
nojail_folder = f"{folder}/no_jailbreak"
output_dir = f"{base_folder}/analysis"
nojail_output_dir = f"{output_dir}/no_jailbreak"
log_files = folder
